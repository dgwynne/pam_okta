...
static int			 errors;
static struct okta_config	*conf;
...
socket		: SOCKET string {
			if (conf->sockname != NULL) {
				yyerror("socket is already configured");
				free($2);
				YYERROR;
			}
...
host		: HOST string {
			if (conf->host != NULL) {
				yyerror("host is already configured");
				free($2);
				YYERROR;
			}
...
user		: USER string {
			if (conf->user != NULL) {
				yyerror("user is already configured");
				free($2);
				YYERROR;
			}
...
lungetc(int c)
{
	...
	if (file->ungetpos >= file->ungetsize) {
		size_t len = file->ungetsize * 2;
		if (len <= file->ungetsize)
			errx(1, "%s size", __func__);

		void *p = realloc(file->ungetbuf, len);
		if (p == NULL)
			err(1, "%s", __func__);
		file->ungetbuf = p;
		file->ungetsize = len;
...
parse_config(const char *filename)
{
	...
	conf = calloc(1, sizeof(struct okta_config));
	if (conf == NULL)
		return (NULL);
	...
	if (conf->user == NULL) {
		conf->user = strdup(OKTAD_USERNAME);
		if (conf->user == NULL)
			err(1, NULL);
	}
	if (conf->host == NULL)
...
void
clear_config(struct okta_config *c)
{
	free(c->user);
	free(c->sockname);
	free(c->host);
...
void
dump_config(const struct okta_config *c)
{
	printf("user \"%s\"\n", c->user);
	printf("socket \"%s\"\n", c->sockname);
	printf("\n");
